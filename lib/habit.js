...
class Habit {
  ...
  //or string input is not exepcted OR has 0 length OR contains chars other than alphabetic or whitespace
  //or other data types are input, e.g. boolean                
  invalidInput(input, type) {
    if (type === "number") {
      return typeof input !== type || (isNaN(input)) || input < 0;
    } else if (type === "string") {
      return typeof input !== type || input.length === 0 || !(/^[A-Za-z\s]+$/.test(input.trim()));
    } else {
      return true;
    }
  }
